...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.customerStatusChange.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.customer-status-changes.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('old_status') ? 'is-invalid' : '' }}" name="old_status_id" id="old_status_id" required>
                    @foreach($old_statuses as $id => $old_status)
                        <option value="{{ $id }}" {{ old('old_status_id') == $id ? 'selected' : '' }}>{{ $old_status }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('new_status') ? 'is-invalid' : '' }}" name="new_status_id" id="new_status_id" required>
                    @foreach($new_statuses as $id => $new_status)
                        <option value="{{ $id }}" {{ old('new_status_id') == $id ? 'selected' : '' }}>{{ $new_status }}</option>
                    @endforeach
                </select>
